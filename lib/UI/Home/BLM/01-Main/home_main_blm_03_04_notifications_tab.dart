...
class HomeBLMNotificationsTabState extends State<HomeBLMNotificationsTab> with AutomaticKeepAliveClientMixin<HomeBLMNotificationsTab>{
  Future<List<APIBLMHomeTabNotificationExtended>>? showListOfNotifications;
  ...
  Future<List<APIBLMHomeTabNotificationExtended>> getListOfNotifications({required int page}) async{
    APIBLMHomeTabNotificationMain? newValue;
    List<APIBLMHomeTabNotificationExtended> listOfNotifications = [];

    do{
      newValue = await apiBLMHomeNotificationsTab(page: page).onError((error, stackTrace){
        showDialog(
          context: context,
      ...
      listOfNotifications.addAll(newValue.blmNotification);
      ...
      builder: (_, bool isGuestLoggedInListener, __) => ValueListenableBuilder(
        ...
        builder: (_, int lengthOfFeedsListener, __) => ValueListenableBuilder(
          ...
          builder: (_, bool loadedListener, __) => RefreshIndicator(
            onRefresh: onRefresh,
            child: FutureBuilder<List<APIBLMHomeTabNotificationExtended>>(
                  ...
                  if(loadedListener && lengthOfFeedsListener == 0){
                    ...
                    return ListView.separated(
                      ...
                      itemBuilder: (c, i) => MiscNotificationDisplayTemplate(
                        ...
                        imageOnPressed: (){
                          Navigator.push(context, MaterialPageRoute(builder: (context) => HomeBLMUserProfile(userId: notifications.data![i].homeTabNotificationActor.homeTabNotificationActorId, accountType: notifications.data![i].homeTabNotificationActor.homeTabNotificationActorAccountType)));
                        },
                        titleOnPressed: (){
                          Navigator.push(context, MaterialPageRoute(builder: (context) => HomeBLMUserProfile(userId: notifications.data![i].homeTabNotificationActor.homeTabNotificationActorId, accountType: notifications.data![i].homeTabNotificationActor.homeTabNotificationActorAccountType)));
                        },
                        notificationOnPressed: () async{
                          if(notifications.data![i].homeTabNotificationNotificationType == 'Memorial'){
                            ...
                            var memorialProfile = await apiBLMShowMemorial(memorialId: notifications.data![i].homeTabNotificationPostId);
                            context.loaderOverlay.hide();

                            Navigator.push(context, MaterialPageRoute(builder: (context) => HomeBLMMemorialProfile(memorialId: notifications.data![i].homeTabNotificationPostId, pageType: notifications.data![i].homeTabNotificationNotificationType, newJoin: memorialProfile.blmMemorial.memorialFollower,)));
                            ...
                            Navigator.push(context, MaterialPageRoute(builder: (context) => HomeBLMShowOriginalPostComments(postId: notifications.data![i].homeTabNotificationPostId)));
                          }
                        },
                      ),
                    );
