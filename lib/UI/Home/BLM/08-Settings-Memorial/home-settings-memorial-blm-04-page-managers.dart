...
class HomeBLMPageManagers extends StatefulWidget {
  ...
  HomeBLMPageManagersState createState() =>
      HomeBLMPageManagersState(memorialId: memorialId);
}
...
class HomeBLMPageManagersState extends State<HomeBLMPageManagers> {
  ...
  void onLoading1() async {
    if (adminItemsRemaining != 0) {
      ...
      var newValue =
          await apiBLMShowAdminSettings(memorialId: memorialId, page: page1);
      ...
      for (int i = 0; i < newValue.blmAdminList.length; i++) {
        managers.add(
          ListTile(
            ...
            subtitle: Text(
                '${newValue.blmAdminList[i].showAdminsSettingsUser.showAdminsSettingsUserEmail}',
              style: TextStyle(
            ...
            trailing: MaterialButton(
              ...
              onPressed: () async {
                ...
                if (confirmation) {
                  context.loaderOverlay.show();
                  String result = await apiBLMDeleteMemorialAdmin(
                      pageType: 'Blm',
                      pageId: memorialId,
                      userId: newValue.blmAdminList[i].showAdminsSettingsUser
  ...
  void onLoading2() async {
    if (familyItemsRemaining != 0) {
      context.loaderOverlay.show();
      var newValue =
          await apiBLMShowAdminSettings(memorialId: memorialId, page: page2);
      context.loaderOverlay.hide();
      ...
      for (int i = 0; i < newValue.blmFamilyList.length; i++) {
        managers.add(
          ListTile(
            ...
            subtitle: Text(
                '${newValue.blmFamilyList[i].showAdminsSettingsUser.showAdminsSettingsUserEmail}',
              style: TextStyle(
                fontSize: SizeConfig.blockSizeVertical! * 2.11,
                fontFamily: 'NexaRegular',
                color: const Color(0xffBDC3C7),
            ...
            trailing: MaterialButton(
              ...
              onPressed: () async {
                ...
                if (confirmation) {
                  context.loaderOverlay.show();
                  String result = await apiBLMAddMemorialAdmin(
                      pageType: 'Blm',
                      pageId: memorialId,
                      userId: newValue.blmFamilyList[i].showAdminsSettingsUser
                          .showAdminsSettingsUserId);
                  context.loaderOverlay.hide();
